...
// ═══════════════════════════════════════════════════════════════
// SUPABASE — Optimised for Asia (low latency saves)
// ═══════════════════════════════════════════════════════════════
const SUPABASE_URL  = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_ANON = import.meta.env.VITE_SUPABASE_ANON_KEY;
const supabase = createClient(SUPABASE_URL, SUPABASE_ANON, {
  auth: {
    // Keep session in localStorage — no repeated auth roundtrips
    persistSession: true,
    autoRefreshToken: true,
    detectSessionInUrl: false,
  },
  db: {
    // Use prefer=representation only when we need data back
    // This avoids double round-trips on simple inserts
    schema: 'public',
  },
  global: {
    headers: {
      // Tell PostgREST we prefer minimal response on mutations
      // reduces payload size = faster on mobile/slow connections
      'X-Client-Info': 'kc-sarswat-erp/4.0',
    },
    // Aggressive fetch timeout — fail fast, don't hang forever
    fetch: (url, options = {}) => {
  ...
  realtime: {
    // Only connect realtime when tab is visible — saves battery + bandwidth
