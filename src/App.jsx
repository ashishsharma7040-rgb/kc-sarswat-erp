...
// ── Nuke entire collection (batch delete all docs) ──────────────────────────────
const fbNukeCollection = async (col) => {
  try {
    const snap = await getDocs(collection(db, col));
    if (snap.empty) return true;
    // Firestore batch max 500 docs at a time
    const chunks = [];
...
const fbNukeAll = async () => {
  const cols = ["sales", "supply", "expenses", "transport", "staff", "settings", "tanks"];
  await Promise.all(cols.map(c => fbNukeCollection(c)));
};
...
const genMonthly = () => ["Jan", "Feb", "Mar", "Apr", "May", "Jun"].map(m => ({
  month: m, petrol: Math.floor(8000 + Math.random() * 4000), diesel: Math.floor(12000 + Math.random() * 5000),
  profit: Math.floor(80000 + Math.random() * 30000), expenses: Math.floor(15000 + Math.random() * 8000)
}));
const genDaily = () => Array.from({ length: 7 }, (_, i) => {
  const d = new Date(); d.setDate(d.getDate() - (6 - i));
  return { date: d.toLocaleDateString("en-IN", { month: "short", day: "numeric" }), petrol: Math.floor(400 + Math.random() * 300), diesel: Math.floor(600 + Math.random() * 400) };
});
...
const Dashboard = ({ settings, tanks }) => {
  ...
  const profit = today.petrol * settings.petrolPrice * 0.042 + today.diesel * settings.dieselPrice * 0.028;
...
const ReportsView = ({ unitConfig }) => {
  ...
  const pl = { revenue: 1240000, cost: 980000, gross: 260000, opex: 85000, ebitda: 175000, dep: 12000, ebit: 163000, tax: 40750, net: 122250 };
  ...
  const exportCSV = (type) => {
    ...
    else csv = `Lubricant Sales — ${unitLabel}\n\nProduct,Qty,Rate,Amount\nMAK Lubes,180,520,93600\nHP Lubes,145,480,69600\nServo,92,680,62560\nOthers,68,390,26520\n`;
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "balance" && <div style={S.card}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div>
          ...
          {[["Current Assets", ""], ["  Inventory (Fuel+Lube)", "₹3,40,000"], ["  Cash & Bank", "₹2,85,000"], ["  Receivables", "₹62,000"], ["Total Current Assets", "₹6,87,000"], [""], ["Fixed Assets", ""], ["  Plant & Machinery", "₹8,50,000"], ["  Vehicles", "₹4,20,000"], ["  Furniture", "₹85,000"], ["Total Fixed Assets", "₹13,55,000"], [""], ["TOTAL ASSETS", "₹19,57,000"]].map(([k, v], i) => k ? <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: k.startsWith("Total") || k === "TOTAL ASSETS" ? "1px solid rgba(255,255,255,0.08)" : "none" }}><span style={{ color: k.startsWith("Total") || k === "TOTAL ASSETS" ? "#f1f5f9" : k.startsWith("  ") ? "#64748b" : "#94a3b8", fontSize: k === "TOTAL ASSETS" ? 13 : 12, fontWeight: k.startsWith("Total") || k === "TOTAL ASSETS" ? 700 : 400 }}>{k}</span><span style={{ color: k === "TOTAL ASSETS" ? "#f59e0b" : "#f1f5f9", fontSize: 12, fontWeight: k.startsWith("Total") || k === "TOTAL ASSETS" ? 700 : 500, fontFamily: "monospace" }}>{v}</span></div> : <div key={i} style={{ height: 8 }} />)}
        </div>
        <div>
          ...
          {[["Owner's Equity", ""], ["  Capital", "₹8,50,000"], ["  Retained Earnings", "₹2,82,000"], ["Total Equity", "₹11,32,000"], [""], ["Current Liabilities", ""], ["  Trade Payables", "₹1,45,000"], ["  Borrowings", "₹2,00,000"], ["Total Current Liab.", "₹3,45,000"], [""], ["Non-Current Liab.", ""], ["  Term Loans", "₹4,80,000"], ["Total NC Liab.", "₹4,80,000"], [""], ["TOTAL LIAB. & EQUITY", "₹19,57,000"]].map(([k, v], i) => k ? <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: k.startsWith("Total") || k === "TOTAL LIAB. & EQUITY" ? "1px solid rgba(255,255,255,0.08)" : "none" }}><span style={{ color: k.startsWith("Total") || k === "TOTAL LIAB. & EQUITY" ? "#f1f5f9" : k.startsWith("  ") ? "#64748b" : "#94a3b8", fontSize: k === "TOTAL LIAB. & EQUITY" ? 13 : 12, fontWeight: k.startsWith("Total") || k === "TOTAL LIAB. & EQUITY" ? 700 : 400 }}>{k}</span><span style={{ color: k === "TOTAL LIAB. & EQUITY" ? "#3b82f6" : "#f1f5f9", fontSize: 12, fontWeight: k.startsWith("Total") || k === "TOTAL LIAB. & EQUITY" ? 700 : 500, fontFamily: "monospace" }}>{v}</span></div> : <div key={i} style={{ height: 8 }} />)}
        </div>
      </div>
    </div>}
    {tab === "lubricants" && <div style={S.card}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{[["MAK Premium 10W30", 180, 520, 93600, 22], ["HP Lubes 20W50", 145, 480, 69600, 18], ["Servo 5W40", 92, 680, 62560, 25], ["HP Gear Oil", 68, 390, 26520, 15], ["Brake Fluid DOT4", 44, 240, 10560, 20], ["Others", 35, 310, 10850, 12]].map(([p, q, rt, t, m]) => <tr key={p} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          <td style={{ color: "#f1f5f9", fontSize: 13, fontWeight: 600, padding: "10px 12px" }}>{p}</td>
          <td style={{ color: "#f59e0b", fontSize: 13, padding: "10px 12px" }}>{q}</td>
          <td style={{ color: "#64748b", fontSize: 13, padding: "10px 12px", fontFamily: "monospace" }}>₹{rt}</td>
          <td style={{ color: "#34d399", fontSize: 13, fontWeight: 700, padding: "10px 12px", fontFamily: "monospace" }}>₹{t.toLocaleString("en-IN")}</td>
          <td style={{ padding: "10px 12px" }}><span style={{ background: "rgba(16,185,129,0.1)", color: "#34d399", fontSize: 11, padding: "3px 8px", borderRadius: 5, fontWeight: 700 }}>{m}%</span></td>
        </tr>)}
          <tr style={{ borderTop: "2px solid rgba(255,255,255,0.1)", background: "rgba(245,158,11,0.05)" }}><td colSpan={3} style={{ color: "#f59e0b", fontSize: 13, fontWeight: 800, padding: "11px 12px" }}>TOTAL</td><td style={{ color: "#f59e0b", fontSize: 14, fontWeight: 800, padding: "11px 12px", fontFamily: "monospace" }}>₹2,73,690</td><td style={{ color: "#f59e0b", fontSize: 13, fontWeight: 700, padding: "11px 12px" }}>Avg 19%</td></tr>
        </tbody>
      </table>
    </div>}
  </div>;
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "reset" && <div style={{ maxWidth: 520 }}>
      <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 18, padding: 22, marginBottom: 18 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, margin: 0, lineHeight: 1.6 }}>Permanently deletes ALL Firebase cloud data (sales, supply, expenses, staff, transport) AND resets local state to zero. Client starts completely fresh. <b style={{ color: "#f87171" }}>Cannot be undone — Firebase data gone forever.</b></p>
      </div>
