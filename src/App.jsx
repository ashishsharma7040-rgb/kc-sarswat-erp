...
const computeOrderAlert = (usableStock, avgSales, minOrder, maxOrder, availableSpace) => {
  ...
  try {
    ...
    // 3-day forecast: how much will sell in next 3 days
    const forecastNeeded = Math.round(avgSales * 3);
    // Perfect order: fill to cover 7 days sales, but never overflow tank, never under min order
    const idealQty = Math.ceil((avgSales * 7) / 1000) * 1000;
    const tankConstrained = availableSpace > 0 ? Math.min(idealQty, Math.floor(availableSpace / 1000) * 1000) : idealQty;
    const suggestedQty = Math.min(maxOrder, Math.max(minOrder, tankConstrained));
    // Opportunity cost check: how much over tank space?
    const overflow = suggestedQty > availableSpace ? suggestedQty - availableSpace : 0;
    ...
    // 3-day demand breakdown
    const day1 = Math.round(avgSales), day2 = Math.round(avgSales), day3 = Math.round(avgSales);
    return { daysLeft: daysLeft.toFixed(1), runsOut, urgency, suggestedQty, truck, mustLeaveBy, departure, avgSales: Math.round(avgSales), forecastNeeded, overflow, availableSpace, day1, day2, day3, idealQty };
  } catch { return null; }
};
...
// ════════════════════════════════════════════════════════════════════════
// DASHBOARD — reads real Firebase sales data, shows zero when empty
// ════════════════════════════════════════════════════════════════════════
const Dashboard = ({ settings, tanks, unitConfig }) => {
  ...
  useEffect(() => {
    fbGet("sales").then(data => {
      setSalesData(data && data.length > 0 ? data : []);
      setLoading(false);
    });
  }, []);

  // Build daily data from real Firebase sales (last 7 days)
  const daily = useMemo(() => {
...
const LogisticsView = ({ tanks, settings, unitConfig }) => {
  ...
  // Compute usable stock (current - buffer) per tank selection
  const { usableStock, availableSpace } = useMemo(() => {
    const units = selectedUnit === "all" ? Object.keys(uc) : [selectedUnit];
    let usable = 0, space = 0;
    units.forEach(uid => {
      const t = tanks[uid]?.[selectedFuel]; if (!t) return;
      const buf = t.buffer || 0;
      usable += Math.max(0, t.current - buf);
      space += Math.max(0, t.capacity - t.current); // actual free space in tank
    });
    return { usableStock: usable, availableSpace: space };
  }, [selectedUnit, selectedFuel, tanks, uc]);

  // Auto avg daily sales from last 7 days Firebase data
  const autoAvg = useMemo(() => {
    ...
    salesData.filter(s => s.date >= cutoffStr).forEach(s => {
      ...
      Object.entries(s.readings).forEach(([mId, r]) => {
        // find which unit this machine belongs to
  ...
  const alert = useMemo(() =>
    computeOrderAlert(usableStock, avgSales, minOrder, maxOrder, availableSpace),
    [usableStock, avgSales, minOrder, maxOrder, availableSpace]
  );
  ...
  const urgencyBg = { CRITICAL: "rgba(239,68,68,0.08)", HIGH: "rgba(249,115,22,0.08)", MEDIUM: "rgba(245,158,11,0.08)", LOW: "rgba(52,211,153,0.08)" };
  const urgencyLabel = { CRITICAL: "🚨 CRITICAL — Order NOW", HIGH: "⚠️ HIGH — Order Today", MEDIUM: "📋 MEDIUM — Plan Order Soon", LOW: "✅ LOW — Stock Healthy" };
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <p style={{ color: "#334155", fontSize: 13, margin: "0 0 20px" }}>
      {usingAutoAvg ? `📊 Using real sales avg from last 7 days` : customAvg ? "✏️ Using manual override" : "Using estimated default avg"}
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12, marginBottom: 20 }}>
      <StatCard label="Usable Stock" value={`${usableStock.toLocaleString()} L`} sub="After buffer deducted" color="#3b82f6" icon="🛢" />
      <StatCard label="Tank Space Free" value={`${availableSpace.toLocaleString()} L`} sub="Can receive this much" color="#10b981" icon="📥" />
      <StatCard label="Avg Daily Sales" value={`${Math.round(avgSales).toLocaleString()} L`} sub={usingAutoAvg ? "Auto · last 7 days" : "Manual override"} color="#f59e0b" icon="📊" />
      {alert && <StatCard label="Days of Stock Left" value={`${alert.daysLeft} days`} sub={`Runs out ${fmtDt(alert.runsOut)}`} color={urgencyColor[alert.urgency]} icon={alert.urgency === "CRITICAL" ? "🚨" : alert.urgency === "HIGH" ? "⚠️" : alert.urgency === "MEDIUM" ? "📋" : "✅"} />}
    </div>

    {/* Manual override */}
    <div style={{ ...S.card, marginBottom: 18, display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>
      <div style={{ flex: 1, minWidth: 200 }}>
        <Label>Override Daily Avg (L/day)</Label>
        <input type="number" value={customAvg} onChange={e => setCustomAvg(e.target.value)}
          placeholder={`Auto: ${Math.round(autoAvg > 0 ? autoAvg : fallbackAvg)} L/day`} style={S.inp} />
      </div>
      ...
      {usingAutoAvg && <div style={{ padding: "10px 14px", background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: 9 }}>
        <p style={{ color: "#34d399", fontSize: 12, fontWeight: 700, margin: 0 }}>✅ Using real Firebase sales data</p>
      </div>}
    </div>

    {/* 3-Day Forecast */}
    {alert && <div style={{ ...S.card, marginBottom: 18 }}>
      <h3 style={{ color: "#a78bfa", fontSize: 14, fontWeight: 700, margin: "0 0 14px" }}>📅 3-Day Demand Forecast</h3>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
        {[1,2,3].map(d => {
          const dt = new Date(); dt.setDate(dt.getDate() + d);
          const dayName = dt.toLocaleDateString("en-IN", { weekday: "short", month: "short", day: "numeric" });
          const demand = Math.round(avgSales);
          const stockAfter = Math.max(0, usableStock - demand * d);
          const pct = Math.min(100, (stockAfter / Math.max(usableStock, 1)) * 100);
          const col = pct < 20 ? "#ef4444" : pct < 50 ? "#f97316" : "#34d399";
          return <div key={d} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, padding: 14, border: `1px solid ${col}33`, textAlign: "center" }}>
            <p style={{ color: "#64748b", fontSize: 11, fontWeight: 700, margin: "0 0 6px", textTransform: "uppercase" }}>Day {d} — {dayName}</p>
            <p style={{ color: "#f1f5f9", fontSize: 16, fontWeight: 800, margin: "0 0 4px" }}>−{demand.toLocaleString()} L</p>
      ...
      <div style={{ background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.2)", borderRadius: 10, padding: "10px 14px" }}>
        <p style={{ color: "#a78bfa", fontSize: 12, fontWeight: 700, margin: "0 0 2px" }}>3-Day Total Demand: {alert.forecastNeeded.toLocaleString()} L</p>
        <p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Based on {Math.round(avgSales).toLocaleString()} L/day average · {selectedFuel} · {selectedUnit === "all" ? "All Units" : uc[selectedUnit]?.name}</p>
      </div>
    </div>}

    {/* Main Alert Card */}
    {alert ? <div style={{ ...S.card, border: `2px solid ${urgencyColor[alert.urgency]}55`, background: urgencyBg[alert.urgency], marginBottom: 18 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
        <div style={{ width: 56, height: 56, borderRadius: 16, background: `${urgencyColor[alert.urgency]}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>
          {alert.urgency === "CRITICAL" ? "🚨" : alert.urgency === "HIGH" ? "⚠️" : alert.urgency === "MEDIUM" ? "📋" : "✅"}
        </div>
        <div>
          <h2 style={{ color: urgencyColor[alert.urgency], fontSize: 17, fontWeight: 800, margin: "0 0 4px" }}>{urgencyLabel[alert.urgency]}</h2>
          <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>{alert.daysLeft} days remaining · {Math.round(avgSales).toLocaleString()} L/day avg</p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: "🚛 Earliest Departure", value: fmtDt(alert.departure), color: "#3b82f6" },
      ...
      {/* Perfect Order Section */}
      <div style={{ background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.25)", borderRadius: 14, padding: 18, marginBottom: 14 }}>
        <h3 style={{ color: "#34d399", fontSize: 14, fontWeight: 700, margin: "0 0 14px" }}>🎯 Perfect Order Calculation</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 12, marginBottom: 14 }}>
          {[
            { label: "7-Day Ideal Qty", value: `${alert.idealQty.toLocaleString()} L`, sub: "covers 7 days demand", color: "#f1f5f9" },
            { label: "Tank Space Free", value: `${alert.availableSpace.toLocaleString()} L`, sub: "max receivable", color: "#10b981" },
            { label: "✅ Order This Much", value: `${alert.suggestedQty.toLocaleString()} L`, sub: "optimised order", color: "#34d399", big: true },
            { label: "Truck Needed", value: alert.truck.name.split("(")[0].trim(), sub: `${alert.truck.capacity.toLocaleString()} L capacity`, color: "#f59e0b" },
          ].map(item => <div key={item.label} style={{ background: item.big ? "rgba(52,211,153,0.1)" : "rgba(255,255,255,0.03)", borderRadius: 10, padding: "12px 14px", border: item.big ? "1px solid rgba(52,211,153,0.3)" : "1px solid rgba(255,255,255,0.06)" }}>
            <p style={{ color: "#64748b", fontSize: 10, fontWeight: 700, margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.4px" }}>{item.label}</p>
            <p style={{ color: item.color, fontSize: item.big ? 20 : 15, fontWeight: 800, margin: "0 0 2px" }}>{item.value}</p>
            <p style={{ color: "#475569", fontSize: 10, margin: 0 }}>{item.sub}</p>
          </div>)}
        </div>
        {alert.overflow > 0 && <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 10, padding: "10px 14px" }}>
          <p style={{ color: "#f59e0b", fontSize: 12, fontWeight: 700, margin: "0 0 2px" }}>⚠ Opportunity Cost Notice</p>
          <p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Ideal 7-day qty ({alert.idealQty.toLocaleString()} L) exceeds available tank space by {alert.overflow.toLocaleString()} L. Order capped to tank capacity to avoid overflow.</p>
        </div>}
        {alert.suggestedQty <= availableSpace && <div style={{ background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.2)", borderRadius: 10, padding: "10px 14px" }}>
          <p style={{ color: "#34d399", fontSize: 12, fontWeight: 700, margin: "0 0 2px" }}>✅ No Opportunity Cost</p>
          <p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Order fits within tank capacity. No overflow risk.</p>
        </div>}
        {/* Compartments */}
        <div style={{ marginTop: 14 }}>
          <p style={{ color: "#64748b", fontSize: 11, fontWeight: 700, margin: "0 0 8px", textTransform: "uppercase" }}>Truck Compartments</p>
          ...
          {alert.truck.preferred && <p style={{ color: "#059669", fontSize: 12, margin: "8px 0 0", fontWeight: 600 }}>✓ Using your personal truck (preferred)</p>}
        </div>
      </div>

      {/* Logistics rules */}
      <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: 12, padding: "12px 16px" }}>
        ...
        <p style={{ color: "#64748b", fontSize: 12, margin: 0, lineHeight: 1.8 }}>
          • Travel: 7h each way · Depot closes 5:00 PM<br />
          • No driving: 11 PM – 6 AM · Closed Sundays & holidays<br />
          • Orders: multiples of 1,000 L · Min {minOrder.toLocaleString()} L · Max {maxOrder.toLocaleString()} L<br />
          • Buffer stock deducted per-tank before usable stock calc
        </p>
      </div>
    </div>
    : <div style={{ ...S.card, textAlign: "center", padding: "50px 20px" }}>
      ...
      {usableStock === 0 && <p style={{ fontSize: 12, color: "#475569", margin: "8px 0 0" }}>Add current tank levels in Settings → Tanks to enable alerts</p>}
    </div>}
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "lubricants" && <div style={S.card}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody><tr><td colSpan={5} style={{ color: "#475569", fontSize: 13, textAlign: "center", padding: "40px 0", fontStyle: "italic" }}>No lubricant sales data yet.</td></tr></tbody>
      </table>
    </div>}
  </div>;
};
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "tanks" && <div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(290px,1fr))", gap: 14 }}>{Object.entries(lt).filter(([k]) => k !== "_fbId" && k !== "savedAt").map(([uid]) => <div key={uid} style={{ ...S.card, border: "1px solid rgba(245,158,11,0.15)" }}>
      ...
      {[["petrol", "⛽ Petrol", "#f59e0b"], ["diesel", "🔵 Diesel", "#3b82f6"]].map(([fk, fname, col]) => <div key={fk} style={{ marginBottom: 12, padding: 12, background: "rgba(255,255,255,0.03)", borderRadius: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}><div style={{ flex: 1 }}><Label>Capacity (L)</Label><input type="number" value={lt[uid]?.[fk]?.capacity || 0} onChange={e => updT(uid, fk, "capacity", e.target.value)} style={S.inp} /></div><div style={{ flex: 1 }}><Label>Current (L)</Label><input type="number" value={lt[uid]?.[fk]?.current || 0} onChange={e => updT(uid, fk, "current", e.target.value)} style={S.inp} /></div><div style={{ flex: 1 }}><Label>Buffer (L)</Label><input type="number" value={lt[uid]?.[fk]?.buffer ?? 1000} onChange={e => updT(uid, fk, "buffer", e.target.value)} style={{ ...S.inp, borderColor: "rgba(239,68,68,0.3)" }} /><p style={{ color:"#ef4444", fontSize:10, margin:"3px 0 0" }}>Dead/unusable stock</p></div></div>
      </div>)}
    </div>)}</div></div>}
    ...
    {tab === "logistics" && <div style={{ ...S.card, maxWidth: 460 }}>
      ...
      <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 10, padding: "10px 14px", marginBottom: 16 }}>
        <p style={{ color: "#f87171", fontSize: 12, fontWeight: 700, margin: "0 0 4px" }}>ℹ Buffer/Dead Stock</p>
        <p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Buffer stock is now set per-tank individually in the <b style={{ color: "#f1f5f9" }}>🪣 Tanks</b> tab above.</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
    ...
    {tab === "info" && <div style={{ ...S.card, maxWidth: 460 }}>
      ...
      {[["Station", "Shree K C Sarswat Auto Fuel Station"], ["Location", "Lunkaransar, Rajasthan 334603"], ["Units", `${Object.keys(luc).length} Units · ${Object.values(luc).reduce((a, u) => a + (u.machines || []).length, 0)} Machines`], ["Distributor", "HPCL"], ["Firebase", "kc-sarswat-erp-9dced"], ["Owner Login", "owner@kcsarswat.in"], ["Built By", "Ashish Sarswat"]].map(([k, v]) => <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}><span style={{ color: "#475569", fontSize: 12, fontWeight: 600 }}>{k}</span><span style={{ color: "#94a3b8", fontSize: 12 }}>{v}</span></div>)}
    </div>}
