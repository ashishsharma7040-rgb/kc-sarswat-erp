...
const fbNukeCollection = async (col) => {
  try {
    ...
    snap.docs.forEach(d => { batch.delete(d.ref); count++; if (count === 499) { batches.push(batch); batch = writeBatch(db); count = 0; } });
    batches.push(batch);
...
const INITIAL_TANKS = {
  unit1: { petrol: { capacity: 10000, current: 0 }, diesel: { capacity: 15000, current: 0 } },
  unit2: { petrol: { capacity: 8000, current: 0 }, diesel: { capacity: 12000, current: 0 } },
  unit3: { petrol: { capacity: 8000, current: 0 }, diesel: { capacity: 12000, current: 0 } },
};
const INITIAL_STAFF = [];
...
const Dashboard = ({ settings, tanks }) => {
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 14, marginBottom: 20 }}>
      ...
      <StatCard label="Total Litres" value={`${(today.petrol + today.diesel).toLocaleString()} L`} sub="All units" color="#10b981" icon="📦"  />
      <StatCard label="Gross Profit" value={`₹${profit.toLocaleString("en-IN", { maximumFractionDigits: 0 })}`} sub="After margins" color="#8b5cf6" icon="💰"  />
    </div>
...
const ExpensesView = () => {
  ...
  const [editId, setEditId] = useState(null), [form, setForm] = useState(ef), [errors, setErrors] = useState([]);
  const [fe, setFe] = useState({}), [saved, setSaved] = useState(false), [saving, setSaving] = useState(false);
  useEffect(() => { fbGet("expenses").then(data => { if (data && data.length > 0) setExpenses(data.sort((a, b) => b.date.localeCompare(a.date))); else setExpenses([]); setLoading(false); }); }, []);
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "lubricants" && <div style={S.card}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody><tr><td colSpan={5} style={{ color: "#475569", fontSize: 13, textAlign: "center", padding: "40px 0", fontStyle: "italic" }}>No lubricant data yet. Add entries to track lubricant sales.</td></tr></tbody>
      </table>
    </div>}
  </div>;
};
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    {tab === "reset" && <div style={{ maxWidth: 520 }}>
      <div style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 18, padding: 22, marginBottom: 18 }}>
        ...
        <p style={{ color: "#64748b", fontSize: 13, margin: 0, lineHeight: 1.6 }}>Permanently deletes ALL Firebase cloud data AND resets app to zero. <b style={{ color: "#f87171" }}>Cannot be undone.</b></p>
      </div>
