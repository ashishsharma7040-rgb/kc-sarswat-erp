...
export default function App() {
  ...
  // Restore session — 4s hard cap, NEVER hangs forever
  useEffect(() => {
    let done = false;
    const finish = () => { if (!done) { done = true; setSessionChecked(true); } };

    // Hard timeout — show login after 4s no matter what
  ...
  // Show loading spinner briefly while checking session
  if (!sessionChecked && !user) {
    return (
      <div style={{ minHeight:"100dvh", display:"flex", alignItems:"center", justifyContent:"center", background:"#060d18", fontFamily:"'Sora',system-ui,sans-serif" }}>
        <div style={{ textAlign:"center" }}>
          <div style={{ fontSize:56, marginBottom:14 }}>⛽</div>
          <p style={{ color:"#f1f5f9", fontSize:15, fontWeight:700, margin:"0 0 6px" }}>KC Sarswat ERP</p>
          <p style={{ color:"#475569", fontSize:12, margin:"0 0 18px" }}>Loading…</p>
