// KC Sarswat Auto Fuel Station — ERP v2.0
// Full Firebase real-time sync | Shift handoff | Cash reconciliation | ICAI Lubricants
...
import { getFirestore, collection, doc, getDocs, setDoc, addDoc, updateDoc, deleteDoc, writeBatch, onSnapshot } from "firebase/firestore";
...
const INIT_SETTINGS = { petrolPrice: 102.84, dieselPrice: 89.62, lowStockAlert: 500, minOrderLitres: 12000, maxOrderLitres: 23000 };
const INIT_TANKS = {
...
const inp = { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "11px 14px", color: "#f1f5f9", fontSize: 13, width: "100%", outline: "none", fontFamily: "inherit", boxSizing: "border-box" };
const inpErr = { ...inp, background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.55)" };
const btn = (c="#f59e0b") => ({ background: `linear-gradient(135deg,${c},${c}cc)`, color: "#fff", border: "none", borderRadius: 12, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" });
const ghostBtn = { background: "rgba(255,255,255,0.05)", color: "#64748b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 22px", fontSize: 13, cursor: "pointer", fontFamily: "inherit" };
...
const ConfirmModal = ({ open, title, msg, onOk, onCancel, okLabel = "Proceed", danger = false, extraBtn }) => {
  ...
  return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
    <div style={{ ...card, border: `1px solid ${danger ? "rgba(239,68,68,0.4)" : "rgba(245,158,11,0.3)"}`, width: 440, boxShadow: "0 20px 60px rgba(0,0,0,0.8)" }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        {extraBtn && <button onClick={extraBtn.onClick} style={{ flex: 1, background: "linear-gradient(135deg,#3b82f6,#2563eb)", color: "#fff", border: "none", borderRadius: 10, padding: "12px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{extraBtn.label}</button>}
...
const DiscrepancyModal = ({ open, field, prevVal, currVal, onContinue, onFix }) => {
  ...
  return <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
    <div style={{ ...card, border: "2px solid rgba(239,68,68,0.6)", width: 480 }}>
      ...
      <p style={{ color: "#f87171", fontSize: 12, fontWeight: 700, margin: "0 0 14px" }}>⛔ ICAI Rule: Opening must equal previous closing. This will be flagged in owner's log.</p>
      <div style={{ display: "flex", gap: 10 }}>
...
// Stat Card
const StatCard = ({ label, value, sub, color = "#f59e0b", icon, loading }) => {
  if (loading) return <div style={{ ...card, height: 100 }}><Skeleton /><Skeleton w="60%" h={24} /><Skeleton w="40%" h={12} /></div>;
...
const TankGauge = ({ label, current, capacity, color, lowAlert, buffer }) => {
  ...
  return <div style={{ ...card, textAlign: "center", padding: "14px 10px" }}>
...
const LoginPage = ({ onLogin, staffList }) => {
  ...
  return <div style={{ minHeight: "100vh", display: "flex", fontFamily: "'Sora',system-ui,sans-serif", background: "#060d18" }}>
    <div style={{ flex: 1, background: "linear-gradient(180deg,#0a0f1a,#0d1c2e,#102238)", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 20 }}>
          {[["⛽","Petrol","#f59e0b"],["🔵","Diesel","#3b82f6"],["🛢","Lubes","#8b5cf6"]].map(([icon,label,col]) =>
            <div key={label} style={{ padding: "8px 16px", background: `rgba(${col === "#f59e0b" ? "245,158,11" : col === "#3b82f6" ? "59,130,246" : "139,92,246"},0.1)`, border: `1px solid ${col}44`, borderRadius: 10 }}>
    ...
    <div style={{ width: 400, background: "#07111e", borderLeft: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", justifyContent: "center", padding: "44px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        ...
        <div><p style={{ color: "#f1f5f9", fontSize: 15, fontWeight: 800, margin: 0 }}>KC Sarswat ERP</p><p style={{ color: "#475569", fontSize: 11, margin: 0 }}>Station Management</p></div>
      </div>
      <h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>Sign In</h1>
      <p style={{ color: "#475569", fontSize: 13, margin: "0 0 22px" }}>Access your dashboard</p>
      ...
      <div style={{ marginBottom: 14 }}>
        <Label>Email</Label>
        <input value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && login()} placeholder={role === "owner" ? "owner@kcsarswat.in" : "yournamekc@gmail.com"} style={{ ...inp, fontSize: 14 }} />
      </div>
      ...
      <button onClick={login} disabled={loading} style={{ width: "100%", background: loading ? "rgba(245,158,11,0.4)" : "linear-gradient(135deg,#f59e0b,#d97706)", color: "#fff", border: "none", borderRadius: 12, padding: "14px 0", fontSize: 14, fontWeight: 800, cursor: loading ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
      ...
      <div style={{ marginTop: 16, padding: "10px 14px", background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.15)", borderRadius: 10 }}>
        <p style={{ color: "#60a5fa", fontSize: 10, fontWeight: 700, margin: "0 0 4px" }}>STAFF FORMAT</p>
        <p style={{ color: "#475569", fontSize: 11, margin: 0, lineHeight: 1.7 }}>Email: <span style={{ color: "#93c5fd" }}>firstnamelastnamekc@gmail.com</span><br />Pwd: <span style={{ color: "#93c5fd" }}>firstnamelastname@123</span></p>
      </div>
    </div>
  </div>;
};
...
const Sidebar = ({ active, setActive, user, onLogout, collapsed, setCollapsed }) => {
  ...
  return <div style={{ width: collapsed ? 64 : 224, background: "linear-gradient(180deg,#06111e,#08192a)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", transition: "width 0.25s", overflow: "hidden", flexShrink: 0, height: "100vh" }}>
    <div style={{ padding: collapsed ? "14px 12px" : "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {!collapsed && <div style={{ flex: 1 }}><p style={{ color: "#f1f5f9", fontSize: 12, fontWeight: 800, margin: 0 }}>KC Sarswat</p><p style={{ color: "#334155", fontSize: 10, margin: 0 }}>Lunkaransar</p></div>}
...
const Dashboard = ({ settings, tanks }) => {
  const [salesData, setSalesData] = useState([]), [lubeData, setLubeData] = useState([]), [loading, setLoading] = useState(true);
  useEffect(() => {
  ...
  const daily = useMemo(() => Array.from({ length: 7 }, (_, i) => {
    ...
    lubeData.filter(l => l.date === ds).forEach(l => lr += (l.price || 0) * (l.qty || 0));
    return { date: d.toLocaleDateString("en-IN", { month: "short", day: "numeric" }), petrol: Math.round(p), diesel: Math.round(di), lubeRev: Math.round(lr) };
  }), [salesData, lubeData]);
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <p style={{ color: "#334155", fontSize: 13, margin: "0 0 20px" }}>{new Date().toLocaleDateString("en-IN", { weekday: "long", year: "numeric", month: "long", day: "numeric" })} · <span style={{ color: "#34d399", fontWeight: 700 }}>🔴 Live</span></p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 14, marginBottom: 20 }}>
      ...
      <StatCard label="Cash Diff" value={`${cashToday.diff >= 0 ? "+" : ""}₹${cashToday.diff.toFixed(0)}`} sub="Short / Over" color={cashToday.diff === 0 ? "#10b981" : cashToday.diff > 0 ? "#10b981" : "#ef4444"} icon="🏦" loading={loading} />
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
      <div style={card}>
        <h3 style={{ color: "#f1f5f9", fontSize: 14, fontWeight: 700, margin: "0 0 14px" }}>7-Day Sales</h3>
      ...
      <div style={card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(90px,1fr))", gap: 8 }}>
          {Object.entries(UNIT_CONFIG).map(([uid, u]) => [
            <TankGauge key={uid + "p"} label={`${u.name} P`} current={tanks[uid]?.petrol?.current || 0} capacity={tanks[uid]?.petrol?.capacity || 10000} color="#f59e0b" lowAlert={settings.lowStockAlert} buffer={tanks[uid]?.petrol?.buffer || 0} />,
            <TankGauge key={uid + "d"} label={`${u.name} D`} current={tanks[uid]?.diesel?.current || 0} capacity={tanks[uid]?.diesel?.capacity || 15000} color="#3b82f6" lowAlert={settings.lowStockAlert} buffer={tanks[uid]?.diesel?.buffer || 0} />
          ])}
        </div>
      </div>
    </div>
...
const SalesView = ({ user, tanks, setTanks, settings }) => {
  ...
  const performSave = async (mode = "new") => {
    ...
    const entry = { date, shift, staffName: user.name, staffId: user.staffId || "owner", readings, phonePeOpen: ppOpen, phonePeClose: ppClose, paytmOpen: ptOpen, paytmClose: ptClose, cashDeposited: parseFloat(cashDep || 0), phonePeSales: ppSales, paytmSales: ptSales, expectedCash, cashDiff, totalFuelRevenue: totalFuelRev, autoTransferUsed: autoXfer, hasDiscrepancy: Math.abs(cashDiff) > 0.01 };

    if (mode === "replace" && existEntry?._fbId) await fbUpdate("sales", existEntry._fbId, entry);
    else if (mode === "merge" && existEntry?._fbId) await fbUpdate("sales", existEntry._fbId, { ...existEntry, ...entry });
    else await fbAdd("sales", entry);
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
      <div>
        <h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>Sales Entry</h1>
        <p style={{ color: "#334155", fontSize: 13, margin: 0 }}>Meter readings + Cash reconciliation · Auto tank deduction</p>
      </div>
    ...
    {prevShift && !autoXfer && <div style={{ ...card, border: "1px solid rgba(52,211,153,0.3)", marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div><p style={{ color: "#34d399", fontSize: 13, fontWeight: 700, margin: "0 0 2px" }}>🔄 Previous Shift Available</p><p style={{ color: "#64748b", fontSize: 12, margin: 0 }}>Auto-fill opening from {prevShift.shift} · {prevShift.date}</p></div>
        <button onClick={autoTransfer} style={{ background: "linear-gradient(135deg,#10b981,#059669)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>✅ Auto-Transfer → Opening</button>
      </div>
    </div>}
    {autoXfer && <Toast msg="✅ Auto-transfer applied — previous closing used as opening" />}

    <ErrBanner errors={errs} />
    <Toast msg={saved ? `✓ Saved — ${date} · ${shift} · Tank updated` : null} />
                ...
                {[{ label: "⛽ Petrol", ok: "petrolOpen", ck: "petrolClose", s: ps, c: "#f59e0b" }, { label: "🔵 Diesel", ok: "dieselOpen", ck: "dieselClose", s: ds, c: "#3b82f6" }].map(f =>
                  <div key={f.label}>
                    ...
                    <div style={{ display: "flex", gap: 5 }}>
                      <div style={{ flex: 1 }}><label style={{ color: "#475569", fontSize: 10, display: "block", marginBottom: 2 }}>Open</label><input type="number" placeholder="0.00" value={r[f.ok]} onChange={e => upd(mId, f.ok, e.target.value)} disabled={locked} style={fe[mId + f.ok] ? { ...inpErr, padding: "8px 10px" } : { ...inp, padding: "8px 10px" }} /></div>
                      <div style={{ flex: 1 }}><label style={{ color: "#475569", fontSize: 10, display: "block", marginBottom: 2 }}>Close</label><input type="number" placeholder="0.00" value={r[f.ck]} onChange={e => upd(mId, f.ck, e.target.value)} disabled={locked} style={fe[mId + f.ck] ? { ...inpErr, padding: "8px 10px" } : { ...inp, padding: "8px 10px" }} /></div>
                    </div>
    ...
    <div style={{ ...card, marginBottom: 14, border: "1px solid rgba(59,130,246,0.2)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12, marginBottom: 14 }}>
        <div><Label>PhonePe Opening ₹</Label><input type="number" value={ppOpen} onChange={e => { setPpOpen(e.target.value); if (prevShift && Math.abs(parseFloat(prevShift.phonePeClose || 0) - parseFloat(e.target.value)) > 0.01) setDiscModal({ open: true, field: "PhonePe Opening", prev: prevShift.phonePeClose || "0", curr: e.target.value }); }} style={inp} disabled={locked} /></div>
        <div><Label>PhonePe Closing ₹</Label><input type="number" value={ppClose} onChange={e => setPpClose(e.target.value)} style={inp} disabled={locked} /></div>
        <div><Label>Paytm Opening ₹</Label><input type="number" value={ptOpen} onChange={e => { setPtOpen(e.target.value); if (prevShift && Math.abs(parseFloat(prevShift.paytmClose || 0) - parseFloat(e.target.value)) > 0.01) setDiscModal({ open: true, field: "Paytm Opening", prev: prevShift.paytmClose || "0", curr: e.target.value }); }} style={inp} disabled={locked} /></div>
        <div><Label>Paytm Closing ₹</Label><input type="number" value={ptClose} onChange={e => setPtClose(e.target.value)} style={inp} disabled={locked} /></div>
        <div><Label>Cash Deposited ₹</Label><input type="number" value={cashDep} onChange={e => setCashDep(e.target.value)} style={{ ...inp, border: "1px solid rgba(245,158,11,0.3)" }} disabled={locked} /></div>
      </div>
    ...
    <button onClick={handleSave} disabled={saving || locked} style={{ background: saving || locked ? "rgba(245,158,11,0.4)" : "linear-gradient(135deg,#f59e0b,#d97706)", color: "#fff", border: "none", borderRadius: 12, padding: "13px 32px", fontSize: 14, fontWeight: 800, cursor: saving || locked ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
...
const LubricantsView = ({ user }) => {
  ...
  const save = async () => {
    ...
    const entry = { date: form.date, unit: form.unit, product: prod, qty, price, costPrice: cp, revenue: qty * price, cogs: qty * cp, grossProfit: qty * (price - cp), staffName: user.name, staffId: user.staffId || "owner" };
    if (editId) { await fbUpdate("lubricants", editId, entry); setLubes(p => p.map(l => l._fbId === editId ? { ...l, ...entry } : l)); }
    else { const id = await fbAdd("lubricants", entry); setLubes(p => [{ ...entry, _fbId: id }, ...p]); }
    setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 3000);
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
      <div><h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>🛢 Lubricants</h1><p style={{ color: "#334155", fontSize: 13, margin: 0 }}>ICAI AS-9 · Revenue vs COGS · Gross Profit</p></div>
    ...
    <Toast msg={saved ? "Lubricant sale saved 🔴 Live" : null} />
    {show && <div style={{ ...card, border: "1px solid rgba(139,92,246,0.25)", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12 }}>
        ...
        <div><Label req>Qty</Label><input type="number" value={form.qty} onChange={e => { setForm(p => ({ ...p, qty: e.target.value })); setFe(p => ({ ...p, qty: "" })); }} style={fe.qty ? inpErr : inp} /><FieldErr msg={fe.qty} /></div>
        <div><Label req>Selling Price ₹</Label><input type="number" value={form.price} onChange={e => { setForm(p => ({ ...p, price: e.target.value })); setFe(p => ({ ...p, price: "" })); }} style={fe.price ? inpErr : inp} /><FieldErr msg={fe.price} /></div>
        <div><Label>Cost Price ₹</Label><input type="number" value={form.costPrice} onChange={e => setForm(p => ({ ...p, costPrice: e.target.value }))} style={inp} placeholder={form.price ? `Auto: ₹${(parseFloat(form.price) * LUBE_COGS).toFixed(2)}` : "65% default"} /></div>
      </div>
      {form.qty && form.price && <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(139,92,246,0.08)", borderRadius: 10, border: "1px solid rgba(139,92,246,0.2)" }}>
        <p style={{ color: "#a78bfa", fontSize: 12, fontWeight: 700, margin: "0 0 4px" }}>ICAI Preview</p>
        <div style={{ display: "flex", gap: 16, fontSize: 12, flexWrap: "wrap" }}>
          <span>Revenue: <b style={{ color: "#a78bfa" }}>₹{(parseFloat(form.qty || 0) * parseFloat(form.price || 0)).toFixed(2)}</b></span>
          <span>COGS: <b style={{ color: "#f87171" }}>₹{(parseFloat(form.qty || 0) * (form.costPrice ? parseFloat(form.costPrice) : parseFloat(form.price || 0) * LUBE_COGS)).toFixed(2)}</b></span>
          <span>GP: <b style={{ color: "#34d399" }}>₹{(parseFloat(form.qty || 0) * (parseFloat(form.price || 0) - (form.costPrice ? parseFloat(form.costPrice) : parseFloat(form.price || 0) * LUBE_COGS))).toFixed(2)}</b></span>
        </div>
      </div>}
      <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
        <button onClick={save} disabled={saving} style={{ background: "linear-gradient(135deg,#8b5cf6,#7c3aed)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{saving ? "Saving…" : editId ? "Update" : "Save"}</button>
    ...
    {loading ? <div style={{ ...card, textAlign: "center", color: "#475569", padding: 40 }}>Loading…</div> : <div style={card}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 680 }}>
          <thead><tr>{["Date", "Unit", "Product", "Qty", "Price", "COGS", "Revenue", "Gross Profit", ""].map(h => <th key={h} style={{ color: "#334155", fontSize: 11, fontWeight: 700, textAlign: "left", padding: "8px 10px", borderBottom: "1px solid rgba(255,255,255,0.06)", textTransform: "uppercase" }}>{h}</th>)}</tr></thead>
          <tbody>
            ...
            {!lubes.length && <tr><td colSpan={9} style={{ color: "#475569", textAlign: "center", padding: "40px 0" }}>No lubricant sales yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>}
...
const StockView = ({ tanks, settings }) => {
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <p style={{ color: "#334155", fontSize: 13, margin: "0 0 20px" }}>Live tank levels · Auto-updated on sales & supply</p>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(110px,1fr))", gap: 10, marginBottom: 20 }}>
      {Object.entries(UNIT_CONFIG).flatMap(([uid, u]) => [
        <TankGauge key={uid + "p"} label={`${u.name} Petrol`} current={tanks[uid]?.petrol?.current || 0} capacity={tanks[uid]?.petrol?.capacity || 10000} color="#f59e0b" lowAlert={settings.lowStockAlert} buffer={tanks[uid]?.petrol?.buffer || 0} />,
        <TankGauge key={uid + "d"} label={`${u.name} Diesel`} current={tanks[uid]?.diesel?.current || 0} capacity={tanks[uid]?.diesel?.capacity || 15000} color="#3b82f6" lowAlert={settings.lowStockAlert} buffer={tanks[uid]?.diesel?.buffer || 0} />
      ])}
    </div>
    <div style={card}>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{["Unit", "Fuel", "Current", "Buffer", "Usable", "Capacity", "Fill %", "Status"].map(h => <th key={h} style={{ color: "#334155", fontSize: 11, fontWeight: 700, textAlign: "left", padding: "8px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)", textTransform: "uppercase" }}>{h}</th>)}</tr></thead>
          <tbody>
...
const SupplyView = ({ user, tanks, setTanks }) => {
  ...
  const saveSupply = async () => {
    ...
    else { const id = await fbAdd("supply", entry); setSupply(p => [{ ...entry, _fbId: id }, ...p]); }
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <Toast msg={saved ? "Supply saved — Tank updated 🔴 Live" : null} />
    {show && <div style={{ ...card, border: "1px solid rgba(245,158,11,0.2)", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12 }}>
        ...
        <div><Label req>Litres</Label><input type="number" value={form.litres} onChange={e => { setForm(p => ({ ...p, litres: e.target.value })); setFe(p => ({ ...p, litres: "" })); }} style={fe.litres ? inpErr : inp} /><FieldErr msg={fe.litres} /></div>
      ...
      <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
        <button onClick={saveSupply} disabled={saving} style={{ background: "linear-gradient(135deg,#10b981,#059669)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{saving ? "Saving…" : "Save + Update Stock"}</button>
...
const LogisticsView = ({ tanks, settings }) => {
  const [selUnit, setSelUnit] = useState("all"), [selFuel, setSelFuel] = useState("petrol"), [custAvg, setCustAvg] = useState("");
  const [salesData, setSalesData] = useState([]), [loading, setLoading] = useState(true);
  const minO = settings.minOrderLitres || 12000, maxO = settings.maxOrderLitres || 23000;
  useEffect(() => { const u = fbListen("sales", d => { setSalesData(d || []); setLoading(false); }); return u; }, []);

  const { usable, space, cap } = useMemo(() => {
    const units = selUnit === "all" ? Object.keys(UNIT_CONFIG) : [selUnit];
    let u = 0, s = 0, c = 0;
    units.forEach(uid => { const t = tanks[uid]?.[selFuel]; if (!t) return; const buf = t.buffer || 0; u += Math.max(0, t.current - buf); s += Math.max(0, t.capacity - t.current); c += t.capacity; });
    return { usable: u, space: s, cap: c };
  }, [selUnit, selFuel, tanks]);

  const autoAvg = useMemo(() => {
    if (!salesData.length) return 0;
    const cut = new Date(); cut.setDate(cut.getDate() - 7); const cs = cut.toISOString().split("T")[0];
    let tot = 0;
    salesData.filter(s => s.date >= cs).forEach(s => {
      if (!s.readings) return;
      Object.entries(s.readings).forEach(([mId, r]) => {
        const mUnit = Object.entries(UNIT_CONFIG).find(([uid, u]) => (u.machines || []).includes(mId))?.[0];
        if (selUnit !== "all" && mUnit !== selUnit) return;
        if (selFuel === "petrol") { const o = parseFloat(r.petrolOpen || 0), c = parseFloat(r.petrolClose || 0); if (c > o) tot += c - o; }
        else { const o = parseFloat(r.dieselOpen || 0), c = parseFloat(r.dieselClose || 0); if (c > o) tot += c - o; }
      });
    });
    return tot / 7;
  }, [salesData, selUnit, selFuel]);

  const avg = custAvg ? (parseFloat(custAvg) || 0) : (autoAvg > 0 ? autoAvg : (selFuel === "petrol" ? 820 : 1250));
  const daysLeft = avg > 0 ? (usable / avg).toFixed(1) : "∞";
  const runsOut = new Date(Date.now() + parseFloat(daysLeft) * 86400000);
  const urgency = parseFloat(daysLeft) <= 1 ? "CRITICAL" : parseFloat(daysLeft) <= 2 ? "HIGH" : parseFloat(daysLeft) <= 4 ? "MEDIUM" : "LOW";
  const suggestedQty = Math.min(maxO, Math.max(minO, Math.floor(space / 1000) * 1000));
  const truck = TRUCKS.find(t => t.pref && t.cap >= suggestedQty) || TRUCKS.find(t => t.cap >= suggestedQty) || TRUCKS[TRUCKS.length - 1];
  const UC = { CRITICAL: "#ef4444", HIGH: "#f97316", MEDIUM: "#f59e0b", LOW: "#34d399" };
  const UT = { CRITICAL: "🚨 Order NOW", HIGH: "⚠️ Order Today", MEDIUM: "📋 Plan This Week", LOW: "✅ Stock Healthy" };
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <p style={{ color: "#334155", fontSize: 13, margin: "0 0 18px" }}>{autoAvg > 0 && !custAvg ? <span style={{ color: "#34d399" }}>✅ Using real 7-day sales data</span> : custAvg ? <span style={{ color: "#f59e0b" }}>Manual override active</span> : <span>Estimated averages</span>}</p>
    <div style={{ display: "flex", gap: 6, marginBottom: 18, flexWrap: "wrap" }}>
      <span style={{ color: "#64748b", fontSize: 11, fontWeight: 600 }}>Unit:</span>
      {[["all", "All"], ...Object.entries(UNIT_CONFIG).map(([k, u]) => [k, u.name])].map(([v, l]) => <button key={v} onClick={() => setSelUnit(v)} style={tb(selUnit === v)}>{l}</button>)}
      <span style={{ color: "#64748b", fontSize: 11, fontWeight: 600, marginLeft: 8 }}>Fuel:</span>
      {[["petrol", "⛽ Petrol"], ["diesel", "🔵 Diesel"]].map(([v, l]) => <button key={v} onClick={() => setSelFuel(v)} style={{ ...tb(selFuel === v), background: selFuel === v ? (v === "petrol" ? "linear-gradient(135deg,#f59e0b,#d97706)" : "linear-gradient(135deg,#3b82f6,#2563eb)") : "rgba(255,255,255,0.06)" }}>{l}</button>)}
    </div>

    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12, marginBottom: 16 }}>
      <StatCard label="Usable Stock" value={`${usable.toLocaleString()} L`} color={urgency === "CRITICAL" ? "#ef4444" : urgency === "HIGH" ? "#f97316" : "#34d399"} icon="🛢" />
      <StatCard label="Days Left" value={daysLeft} sub={`${Math.round(avg).toLocaleString()} L/day`} color={UC[urgency]} icon="📅" />
      <StatCard label="Order Qty" value={`${suggestedQty.toLocaleString()} L`} sub={truck.name} color="#8b5cf6" icon="🚛" />
    </div>

    <div style={{ ...card, border: `2px solid ${UC[urgency]}55`, marginBottom: 16 }}>
      <h2 style={{ color: UC[urgency], fontSize: 18, fontWeight: 900, margin: "0 0 8px" }}>{UT[urgency]}</h2>
      <p style={{ color: "#64748b", fontSize: 13, margin: "0 0 16px" }}>Runs out: <b style={{ color: UC[urgency] }}>{runsOut.toLocaleDateString("en-IN", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</b></p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>
        {[["Available Space", `${space.toLocaleString()} L`, "#60a5fa"], ["Suggested Order", `${suggestedQty.toLocaleString()} L`, "#34d399"], ["Best Truck", truck.name, truck.pref ? "#f59e0b" : "#f1f5f9"]].map(([l, v, c]) =>
          <div key={l} style={{ padding: "10px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 10 }}>
            <p style={{ color: "#64748b", fontSize: 10, fontWeight: 700, margin: "0 0 4px", textTransform: "uppercase" }}>{l}</p>
    ...
    <div style={{ ...card, marginBottom: 16, display: "flex", gap: 14, alignItems: "flex-end", flexWrap: "wrap" }}>
      <div style={{ flex: 1, minWidth: 200 }}><Label>Override Daily Average (L/day)</Label><input type="number" value={custAvg} onChange={e => setCustAvg(e.target.value)} placeholder={`Auto: ${Math.round(avg)} L/day`} style={inp} /></div>
      {custAvg && <button onClick={() => setCustAvg("")} style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 9, padding: "10px 16px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>✕ Clear</button>}
    </div>

    <div style={card}>
      <h3 style={{ color: "#f1f5f9", fontSize: 14, fontWeight: 700, margin: "0 0 14px" }}>🚛 Available Trucks</h3>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10 }}>
        {TRUCKS.map(t => <div key={t.id} style={{ background: t.pref ? "rgba(245,158,11,0.06)" : "rgba(255,255,255,0.03)", border: `1px solid ${t.pref ? "rgba(245,158,11,0.3)" : "rgba(255,255,255,0.07)"}`, borderRadius: 12, padding: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}><p style={{ color: t.pref ? "#f59e0b" : "#f1f5f9", fontSize: 13, fontWeight: 700, margin: 0 }}>{t.name}</p>{t.pref && <span style={{ background: "rgba(245,158,11,0.15)", color: "#f59e0b", fontSize: 9, padding: "2px 6px", borderRadius: 4, fontWeight: 700 }}>⭐ OWN</span>}</div>
          <p style={{ color: "#475569", fontSize: 11, margin: "4px 0 0" }}>{t.cap.toLocaleString()} L capacity</p>
        </div>)}
      </div>
    </div>
  </div>;
...
const ExpensesView = () => {
  ...
  const [errs, setErrs] = useState([]), [fe, setFe] = useState({}), [saving, setSaving] = useState(false), [saved, setSaved] = useState(false);
  useEffect(() => { const u = fbListen("expenses", d => { setExpenses(d.sort((a, b) => (b.date || "").localeCompare(a.date || ""))); setLoading(false); }); return u; }, []);

  const validate = () => { const e = [], nfe = {}; if (!form.amount || parseFloat(form.amount) <= 0) { e.push("Amount required"); nfe.amount = "Required"; } if (form.category === "Custom" && !form.customCat.trim()) { e.push("Category name required"); nfe.customCat = "Required"; } setErrs(e); setFe(nfe); return e.length === 0; };
  const save = async () => { if (!validate()) return; setSaving(true); const entry = { date: form.date, category: form.category === "Custom" ? form.customCat : form.category, amount: parseFloat(form.amount), note: form.note }; const id = await fbAdd("expenses", entry); setExpenses(p => [{ ...entry, _fbId: id }, ...p]); setForm({ date: TODAY, category: EXPENSE_CATS[0], amount: "", note: "", customCat: "" }); setShow(false); setErrs([]); setFe({}); setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 3000); };
  const total = expenses.reduce((s, e) => s + (e.amount || 0), 0);

  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
      <div><h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>Expenses</h1><p style={{ color: "#334155", fontSize: 13, margin: 0 }}>Owner only · 🔴 Live sync</p></div>
      <div style={{ display: "flex", gap: 8 }}>{(saving || saved) && <SyncBadge saving={saving} />}<button onClick={() => setShow(!show)} style={btn("#ef4444")}>+ Add</button></div>
    </div>
    <StatCard label="Total Expenses" value={`₹${total.toLocaleString("en-IN")}`} sub={`${expenses.length} entries`} color="#ef4444" icon="💸" />
    <Toast msg={saved ? "Expense saved 🔴 Live" : null} />
    {show && <div style={{ ...card, border: "1px solid rgba(239,68,68,0.2)", marginTop: 16, marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12 }}>
        ...
        {form.category === "Custom" && <div><Label req>Name</Label><input value={form.customCat} onChange={e => { setForm(p => ({ ...p, customCat: e.target.value })); setFe(p => ({ ...p, customCat: "" })); }} style={fe.customCat ? inpErr : inp} /><FieldErr msg={fe.customCat} /></div>}
        <div><Label req>Amount ₹</Label><input type="number" value={form.amount} onChange={e => { setForm(p => ({ ...p, amount: e.target.value })); setFe(p => ({ ...p, amount: "" })); }} style={fe.amount ? inpErr : inp} /><FieldErr msg={fe.amount} /></div>
        <div style={{ gridColumn: "1 / -1" }}><Label>Note</Label><input value={form.note} onChange={e => setForm(p => ({ ...p, note: e.target.value }))} style={inp} /></div>
      </div>
      <div style={{ marginTop: 12, display: "flex", gap: 10 }}>
        <button onClick={save} disabled={saving} style={{ background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{saving ? "Saving…" : "Save"}</button>
        <button onClick={() => setShow(false)} style={ghostBtn}>Cancel</button>
      </div>
    </div>}
    {!loading && expenses.length > 0 && <div style={{ ...card, marginTop: 16 }}>
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
        ...
        <tbody>{expenses.map(e => <tr key={e._fbId} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
          ...
          <td style={{ color: "#f87171", fontSize: 13, fontWeight: 700, padding: "10px 12px" }}>₹{e.amount.toLocaleString("en-IN")}</td>
...
const StaffView = ({ staffList, setStaffList }) => {
  const [showForm, setShowForm] = useState(false), [showPwd, setShowPwd] = useState(null), [newPwd, setNewPwd] = useState("");
  const [errs, setErrs] = useState([]), [fe, setFe] = useState({}), [saving, setSaving] = useState(false), [saved, setSaved] = useState(false);
  const [form, setForm] = useState({ name: "", role: "Pump Operator", salary: "", joining: TODAY, unit: "unit1" });

  const validate = () => { const e = [], nfe = {}; if (!form.name.trim()) { e.push("Name required"); nfe.name = "Required"; } if (!form.salary || parseFloat(form.salary) <= 0) { e.push("Salary required"); nfe.salary = "Required"; } setErrs(e); setFe(nfe); return e.length === 0; };
  const addStaff = async () => { if (!validate()) return; setSaving(true); const nm = { name: form.name, role: form.role, salary: parseFloat(form.salary), joining: form.joining, unit: form.unit, active: true, password: makePwd(form.name) }; const id = await fbAdd("staff", nm); setStaffList(p => [...p, { ...nm, _fbId: id, id: id || `s${Date.now()}` }]); setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 3000); setForm({ name: "", role: "Pump Operator", salary: "", joining: TODAY, unit: "unit1" }); setShowForm(false); setErrs([]); setFe({}); };
  const toggle = async id => { const s = staffList.find(x => x._fbId === id || x.id === id); if (!s) return; if (s._fbId) await fbUpdate("staff", s._fbId, { active: !s.active }); setStaffList(p => p.map(x => (x._fbId === id || x.id === id) ? { ...x, active: !x.active } : x)); };
  const savePwd = async id => { const s = staffList.find(x => x._fbId === id || x.id === id); if (s?._fbId) await fbUpdate("staff", s._fbId, { password: newPwd }); setStaffList(p => p.map(x => (x._fbId === id || x.id === id) ? { ...x, password: newPwd } : x)); setShowPwd(null); setNewPwd(""); };
  const payroll = staffList.filter(s => s.active).reduce((a, s) => a + (s.salary || 0), 0);
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>{saved && <SyncBadge saving={false} />}<button onClick={() => setShowForm(!showForm)} style={btn()}>+ Add Staff</button></div>
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 20 }}>
      ...
      <StatCard label="Monthly Payroll" value={`₹${payroll.toLocaleString("en-IN")}`} sub="Active only" color="#ef4444" icon="💸" />
    </div>
    {showForm && <div style={{ ...card, border: "1px solid rgba(245,158,11,0.2)", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12 }}>
        ...
        <div><Label req>Salary ₹</Label><input type="number" value={form.salary} onChange={e => { setForm(p => ({ ...p, salary: e.target.value })); setFe(p => ({ ...p, salary: "" })); }} style={fe.salary ? inpErr : inp} /><FieldErr msg={fe.salary} /></div>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 10 }}>
        <button onClick={addStaff} disabled={saving} style={btn()}>{saving ? "Saving…" : "Add Staff"}</button>
        <button onClick={() => { setShowForm(false); setErrs([]); setFe({}); }} style={ghostBtn}>Cancel</button>
      </div>
    </div>}
...
const TransportView = () => {
  ...
  const save = async () => {
    ...
    const id = await fbAdd("transport", entry); setRecords(p => [{ ...entry, _fbId: id }, ...p]);
    setSaving(false); setSaved(true); setTimeout(() => setSaved(false), 3000);
    ...
    {show && <div style={{ ...card, border: "1px solid rgba(59,130,246,0.2)", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(148px,1fr))", gap: 12 }}>
        {[["date", "Date", "date"], ["truck", "Truck No.", "text"], ["km", "KM", "number"], ["rate", "Rate ₹/km", "number"], ["driverPay", "Driver Pay ₹", "number"], ["diesel", "Diesel Cost ₹", "number"], ["loadingCost", "Loading/Unloading ₹", "number"], ["note", "Note", "text"]].map(([k, l, t]) =>
          <div key={k}><Label>{l}</Label><input type={t} value={form[k]} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} style={inp} /></div>
        )}
      </div>
...
const ReportsView = ({ settings }) => {
  ...
  const pl = useMemo(() => {
    ...
    const netProfit = petrolRev + dieselRev + lubeGP + transIncome - transCost - expenses;
    return { petrolL, dieselL, petrolRev, dieselRev, lubeRev, lubeCOGS, lubeGP, totalRev, expenses, transIncome, transCost, netProfit };
  }, [filtered, settings]);
  ...
  const tabs = [["pl", "📊 P&L"], ["cash", "💰 Cash Log"], ["lubes", "🛢 Lubricants"]];
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
      <div><h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>Reports & Analytics</h1><p style={{ color: "#334155", fontSize: 13, margin: 0 }}>ICAI compliant · 🔴 Live</p></div>
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}><input type="month" value={month} onChange={e => setMonth(e.target.value)} style={{ ...inp, width: 160 }} /><span style={{ color: "#475569", fontSize: 11 }}>Showing {filtered.sales.length} sales</span></div>
    </div>

    <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
      {tabs.map(([id, label]) => <button key={id} onClick={() => setTab(id)} style={{ padding: "8px 16px", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 700, background: tab === id ? "linear-gradient(135deg,#f59e0b,#d97706)" : "rgba(255,255,255,0.06)", color: tab === id ? "#fff" : "#64748b" }}>{label}</button>)}
    </div>
    ...
    {tab === "pl" && <div style={card}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <tbody>
          ...
          <tr><td colSpan={3} style={{ color: "#a78bfa", fontSize: 11, fontWeight: 700, padding: "12px 14px 6px", textTransform: "uppercase", borderTop: "1px solid rgba(255,255,255,0.06)" }}>II. Lubricant Revenue (ICAI)</td></tr>
          ...
          <tr><td colSpan={3} style={{ color: "#f59e0b", fontSize: 11, fontWeight: 700, padding: "12px 14px 6px", textTransform: "uppercase", borderTop: "1px solid rgba(255,255,255,0.06)" }}>III. Other Income</td></tr>
          {row("Transport Income", pl.transIncome)}
          {row("Less: Transport Cost", pl.transCost, false, "#f87171")}
          <tr><td colSpan={3} style={{ color: "#ef4444", fontSize: 11, fontWeight: 700, padding: "12px 14px 6px", textTransform: "uppercase", borderTop: "1px solid rgba(255,255,255,0.06)" }}>IV. Expenses</td></tr>
          {row("Operating Expenses", pl.expenses, false, "#f87171")}
          <tr style={{ borderTop: "2px solid rgba(245,158,11,0.3)" }}>
            <td style={{ color: "#f59e0b", fontSize: 14, fontWeight: 800, padding: "14px" }}>NET PROFIT (Before Tax)</td>
            <td style={{ color: pl.netProfit >= 0 ? "#34d399" : "#f87171", fontSize: 20, fontWeight: 900, padding: "14px", textAlign: "right", fontFamily: "monospace" }}>₹{pl.netProfit.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</td>
          </tr>
        </tbody>
      </table>
    </div>}
    ...
    {tab === "cash" && <div style={card}>
      <h3 style={{ color: "#f59e0b", fontSize: 15, fontWeight: 800, margin: "0 0 4px" }}>Cash Reconciliation Log</h3>
      <p style={{ color: "#475569", fontSize: 12, margin: "0 0 16px" }}>Shifts with discrepancies · flagged for owner</p>
      {cashLog.length === 0 ? <div style={{ textAlign: "center", padding: "40px 0" }}><p style={{ fontSize: 36 }}>✅</p><p style={{ color: "#34d399", fontWeight: 700 }}>No discrepancies this month!</p></div> :
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
          <thead><tr>{["Date", "Shift", "Staff", "PhonePe", "Paytm", "Expected", "Actual", "Difference"].map(h => <th key={h} style={{ color: "#334155", fontSize: 11, fontWeight: 700, textAlign: "left", padding: "8px 10px", borderBottom: "1px solid rgba(255,255,255,0.06)", textTransform: "uppercase" }}>{h}</th>)}</tr></thead>
          <tbody>{cashLog.map(s => <tr key={s._fbId} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)", background: Math.abs(s.cashDiff || 0) > 100 ? "rgba(239,68,68,0.04)" : "transparent" }}>
            <td style={{ color: "#64748b", fontSize: 12, padding: "10px 10px" }}>{s.date}</td>
            <td style={{ color: "#f1f5f9", fontSize: 12, padding: "10px 10px" }}>{s.shift}</td>
            <td style={{ color: "#60a5fa", fontSize: 12, padding: "10px 10px" }}>{s.staffName}</td>
            <td style={{ color: "#64748b", fontSize: 12, padding: "10px 10px" }}>₹{(s.phonePeSales || 0).toFixed(0)}</td>
            <td style={{ color: "#64748b", fontSize: 12, padding: "10px 10px" }}>₹{(s.paytmSales || 0).toFixed(0)}</td>
            <td style={{ color: "#f59e0b", fontSize: 12, padding: "10px 10px" }}>₹{(s.expectedCash || 0).toFixed(0)}</td>
            <td style={{ color: "#f1f5f9", fontSize: 12, padding: "10px 10px" }}>₹{(s.cashDeposited || 0).toFixed(0)}</td>
            <td style={{ padding: "10px 10px" }}><span style={{ color: (s.cashDiff || 0) >= 0 ? "#34d399" : "#f87171", fontSize: 13, fontWeight: 800 }}>{(s.cashDiff || 0) >= 0 ? "+" : ""}₹{(s.cashDiff || 0).toFixed(0)}</span></td>
          </tr>)}</tbody>
        </table></div>}
    </div>}

    {tab === "lubes" && <div style={card}>
      <h3 style={{ color: "#a78bfa", fontSize: 15, fontWeight: 800, margin: "0 0 16px" }}>Lubricant Sales Detail — {month}</h3>
      {filtered.lubes.length === 0 ? <p style={{ color: "#475569", textAlign: "center", padding: "30px 0" }}>No lubricant sales this month.</p> :
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{["Date", "Product", "Qty", "Price", "COGS", "Revenue", "GP"].map(h => <th key={h} style={{ color: "#334155", fontSize: 11, fontWeight: 700, textAlign: "left", padding: "8px 10px", borderBottom: "1px solid rgba(255,255,255,0.06)", textTransform: "uppercase" }}>{h}</th>)}</tr></thead>
          <tbody>
...
const SettingsView = ({ settings, setSettings, tanks, setTanks }) => {
  ...
  const [storageStats, setStorageStats] = useState(null), [loadingStats, setLoadingStats] = useState(false);
  ...
  const fetchStorageStats = async () => {
    ...
    const cols = ["sales", "supply", "expenses", "transport", "staff", "settings", "tanks", "lubricants"];
    let totalDocs = 0, totalChars = 0;
    const breakdown = {};
    for (const col of cols) {
      const docs = await fbGet(col);
      const chars = JSON.stringify(docs || []).length;
      breakdown[col] = { docs: docs?.length || 0, kb: (chars / 1024).toFixed(2) };
      totalDocs += docs?.length || 0; totalChars += chars;
    }
    setStorageStats({ totalDocs, totalKB: (totalChars / 1024).toFixed(2), totalMB: (totalChars / (1024 * 1024)).toFixed(3), breakdown });
    setLoadingStats(false);
  };

  const doNuke = async () => {
    if (nukePwd !== OWNER.pwd) { setNukeErr("Wrong password."); return; }
    await fbNuke("sales"); await fbNuke("supply"); await fbNuke("expenses"); await fbNuke("transport"); await fbNuke("lubricants");
    await fbSet("tanks", "main", INIT_TANKS);
    setTanks(INIT_TANKS); setNukeModal(false); setNukePwd(""); setNukeErr("");
  ...
  return <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
      <div><h1 style={{ color: "#f1f5f9", fontSize: 22, fontWeight: 800, margin: "0 0 4px" }}>Settings</h1><p style={{ color: "#334155", fontSize: 13, margin: 0 }}>Owner only</p></div>
      {(saving || saved) && <SyncBadge saving={saving} />}
    </div>
    ...
    <div style={{ ...card, marginBottom: 16, border: "1px solid rgba(245,158,11,0.2)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
        {[["petrolPrice", "Petrol Price ₹/L"], ["dieselPrice", "Diesel Price ₹/L"], ["lowStockAlert", "Low Stock Alert (L)"], ["minOrderLitres", "Min Order (L)"], ["maxOrderLitres", "Max Order (L)"]].map(([k, l]) =>
          <div key={k}><Label>{l}</Label><input type="number" step="0.01" value={form[k]} onChange={e => setForm(p => ({ ...p, [k]: parseFloat(e.target.value) || 0 }))} style={inp} /></div>
        )}
      </div>
    </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10 }}>
            {["petrol", "diesel"].map(fk => {
              ...
              return ["current", "capacity", "buffer"].map(field =>
                <div key={uid + fk + field}><Label><span style={{ color: col }}>{fk}</span> {field}</Label><input type="number" value={t[field] || 0} onChange={e => setTankForm(p => { const n = JSON.parse(JSON.stringify(p)); if (n[uid]?.[fk]) n[uid][fk][field] = parseFloat(e.target.value) || 0; return n; })} style={inp} /></div>
              );
            })}
          </div>
        </div>
    ...
    <div style={{ ...card, marginBottom: 16, border: "1px solid rgba(59,130,246,0.2)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <h3 style={{ color: "#60a5fa", fontSize: 14, fontWeight: 700, margin: 0 }}>☁️ Firebase Storage Usage</h3>
        <button onClick={fetchStorageStats} disabled={loadingStats} style={{ background: "linear-gradient(135deg,#3b82f6,#2563eb)", color: "#fff", border: "none", borderRadius: 9, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>{loadingStats ? "Loading…" : "📊 Check Usage"}</button>
      </div>
      {storageStats && <div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 10, marginBottom: 12 }}>
          {[{ l: "Total Documents", v: storageStats.totalDocs, c: "#f1f5f9" }, { l: "Total Size", v: `${storageStats.totalKB} KB`, c: "#60a5fa" }, { l: "MB Used", v: `${storageStats.totalMB} MB`, c: "#34d399" }].map(x =>
            <div key={x.l} style={{ padding: "10px 14px", background: "rgba(255,255,255,0.03)", borderRadius: 10 }}>
              <p style={{ color: "#64748b", fontSize: 10, fontWeight: 700, margin: "0 0 4px", textTransform: "uppercase" }}>{x.l}</p>
              <p style={{ color: x.c, fontSize: 16, fontWeight: 800, margin: 0 }}>{x.v}</p>
            </div>
        ...
        <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr>{["Collection", "Docs", "Size"].map(h => <th key={h} style={{ color: "#334155", fontSize: 11, fontWeight: 700, textAlign: "left", padding: "6px 10px", borderBottom: "1px solid rgba(255,255,255,0.06)", textTransform: "uppercase" }}>{h}</th>)}</tr></thead>
          <tbody>{Object.entries(storageStats.breakdown).map(([col, info]) => <tr key={col} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
            <td style={{ color: "#f1f5f9", fontSize: 12, padding: "8px 10px" }}>{col}</td>
            <td style={{ color: "#60a5fa", fontSize: 12, padding: "8px 10px" }}>{info.docs}</td>
            <td style={{ color: "#34d399", fontSize: 12, padding: "8px 10px" }}>{info.kb} KB</td>
          </tr>)}</tbody>
        </table></div>
      </div>}
    </div>
    ...
    <div style={{ ...card, border: "2px solid rgba(239,68,68,0.3)" }}>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button onClick={() => { if (nukePwd !== OWNER.pwd) { setNukeErr("Wrong password."); return; } setNukeModal(true); }} style={{ background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>⚠ Reset</button>
      </div>
...
export default function App() {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#060d18", fontFamily: "'Sora',system-ui,sans-serif", color: "#f1f5f9", overflow: "hidden" }}>
      <style>{`* { box-sizing: border-box; } ::-webkit-scrollbar { width: 4px; height: 4px; } ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 2px; } select option { background: #07111e; color: #f1f5f9; }`}</style>
      <Sidebar active={active} setActive={setActive} user={user} onLogout={onLogout} collapsed={collapsed} setCollapsed={setCollapsed} />
