...
// ═══════════════════════════════════════════════════════════════
// SUPABASE — keys from .env (never hardcoded)
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// SUPABASE DATA LAYER — paginated, filtered queries
// ═══════════════════════════════════════════════════════════════
const db = {
  // SETTINGS
  getSettings: async () => {
  ...
  updateTank: async (unitId, fuelType, current) => {
    await supabase.from("tanks").update({ current, updated_at: new Date().toISOString() }).eq("unit_id", unitId).eq("fuel_type", fuelType);
  },
  ...
  getExistEntry: async (date, shift) => {
    const { data } = await supabase.from("sales").select("*").eq("date", date).eq("shift", shift).maybeSingle();
    return data;
  },
  ...
  insertSupply: async (entry) => {
    const { data, error } = await supabase.from("supply").insert(entry).select().single();
    return error ? null : data;
  },
...
const ConfirmModal = ({ open, title, msg, onOk, onCancel, okLabel="Proceed", danger=false, extraBtn }) => {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999 }}>
      <div style={{ ...card, border:`1px solid ${danger?"rgba(239,68,68,0.4)":"rgba(245,158,11,0.3)"}`, width:440, maxWidth:"95vw", boxShadow:"0 20px 60px rgba(0,0,0,0.8)" }}>
...
const SalesView = () => {
  ...
  const performSave=async(mode="new")=>{
    ...
    for (const [uid,sold] of Object.entries(unitSold)) {
      const pt=tanks?.find(t=>t.unit_id===uid&&t.fuel_type==="petrol"); const dt=tanks?.find(t=>t.unit_id===uid&&t.fuel_type==="diesel");
      if(pt) await db.updateTank(uid,"petrol",Math.max(0,(pt.current||0)-sold.petrol));
      if(dt) await db.updateTank(uid,"diesel",Math.max(0,(dt.current||0)-sold.diesel));
    }
    const freshTanks=await db.getTanks(); setTanks(freshTanks);
    // Cash integrity log
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        const { data: profile } = await supabase.from("staff").select("*").eq("id", session.user.id).maybeSingle();
        if (profile && profile.active) setUser({ ...profile, email: session.user.email });
      }
  ...
  if (authLoading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#060d18", fontFamily:"'Sora',system-ui,sans-serif" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ color:"#475569", fontSize:14 }}>Loading KC Sarswat ERP…</p>
      </div>
    </div>
  );
