...
// Default settings — shown instantly before DB loads, prevents blank fields
const DEFAULT_SETTINGS = {
  station_name: "Shree K C Sarswat Auto Fuel Station",
  address_line1: "Near HPCL Depot", address_line2: "",
  city: "Lunkaransar", state: "Rajasthan", state_code: "08", pincode: "334603",
  gstin: "", pan: "", proprietor_name: "K C Sarswat",
  phone: "", email: "owner@kcsarswat.in",
  bank_name: "", bank_account: "", bank_ifsc: "", bank_branch: "", upi_id: "",
  petrol_price: 102.84, diesel_price: 89.62,
  petrol_margin: 4.27, diesel_margin: 2.72,
  low_stock_alert: 500, monthly_opex: 85000, cash_tolerance: 500,
  state_vat_petrol: 4.27, state_vat_diesel: 2.72,
...
// ═══════════════════════════════════════════════════════════════
// ROOT APP — Bulletproof auth, no loading screen bugs
// ═══════════════════════════════════════════════════════════════
export default function App() {
  const [user, setUser]           = useState(null);
  const [active, setActive]       = useState("dashboard");
  const [collapsed, setCollapsed] = useState(false);
  const [settings, setSettings]   = useState({ ...DEFAULT_SETTINGS, id: "fallback" });
  const [tanks, setTanks]         = useState([]);
  const [ready, setReady]         = useState(false);

  // Load CDN scripts (pdfmake + SheetJS)
  useEffect(() => {
    if (!window.pdfMake) {
  ...
  // Rule: always call setReady(true) no matter what happens
  useEffect(() => {
    let cancelled = false;

    // Hard 4s cap — if anything hangs, we still show login
    const hardCap = setTimeout(() => {
      if (!cancelled) { cancelled = true; setReady(true); }
    }, 4000);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
      if (event === "SIGNED_OUT") { setUser(null); setActive("dashboard"); }
    });
  ...
  useEffect(() => {
    if (!user) return;
    db.getSettings().then(s => { if (s) setSettings(s); });
    db.getTanks().then(t => { if (t?.length) setTanks(t); });
  }, [user]);
  ...
  // NOT ready yet — show brief spinner (max 4s, then login shows)
  if (!ready) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#060d18", fontFamily:"'Sora',system-ui,sans-serif" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <p style={{ color:"#f1f5f9", fontSize:15, fontWeight:700, margin:"0 0 4px" }}>KC Sarswat ERP</p>
        <p style={{ color:"#475569", fontSize:12, margin:"0 0 20px" }}>v4.0</p>
        <div style={{ width:120, height:2, background:"rgba(255,255,255,0.07)", borderRadius:99, overflow:"hidden", margin:"0 auto" }}>
          <div style={{ height:"100%", width:"45%", background:"linear-gradient(90deg,#f59e0b,#d97706)", borderRadius:99, animation:"kcs 1.2s ease-in-out infinite" }}/>
        </div>
        <style>{`@keyframes kcs{0%{transform:translateX(-100%)}100%{transform:translateX(280%)}}`}</style>
      </div>
    </div>
  );
